...
const BiometricSection: React.FC = () => {
  ...
  return (
    <section className="w-full py-24 bg-white overflow-hidden relative font-sans">
      <div className="max-w-[1440px] mx-auto px-6 md:px-20 grid grid-cols-1 lg:grid-cols-2 items-center gap-16 relative z-10">

        {/* --- Ձախ հատված. Անիմացիաներ և դեմքի սկանավորում --- */}
        <div className="relative flex justify-center items-center h-[500px] md:h-[600px]">
          ...
          {/* Դեմքերի հերթափոխ (Identify Image) */}
          <AnimatePresence mode="wait">
            <motion.img
              ...
              className="absolute w-full max-w-[420px] object-contain z-10"
              style={{ filter: 'grayscale(100%) brightness(110%) contrast(110%)' }} 
            />
          </AnimatePresence>

          {/* Սկանավորման ցանց (Overlay) */}
        ...
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
          <h2 className="text-3xl md:text-5xl font-[900] text-[#1a1a1a] mb-8 leading-tight italic uppercase tracking-tighter">
            {t('biometric.title_part1')} <br /> {t('biometric.title_part2')}
          </h2>
          ...
          <div className="flex flex-col md:flex-row items-center gap-12">
            {/* QR Code */}
            <div className="relative p-4 bg-white border border-gray-100 rounded-2xl shadow-2xl group cursor-pointer transition-transform hover:scale-105">
              <img
            ...
            {/* Գործողության կոճակ */}
            <button className="bg-[#6610f2] text-white px-12 py-4 rounded-full font-black text-lg hover:bg-[#520dc2] transition-all transform hover:scale-105 shadow-xl italic uppercase">
              {t('biometric.button')}
            </button>
          </div>
        </div>
