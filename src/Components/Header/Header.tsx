...
const Header: React.FC = () => {
  ...
  const toggleMenu = () => {
    setIsOpen(!isOpen);
    setActiveTab(getCurrentTopPath()); 
  ...
  // Ֆունկցիա, որը գտնում է տվյալ բաժնի առաջին էջը
  const getFirstPath = (base: string) => {
    return secondaryMenus[base]?.[0]?.path || base;
  };

  const topNavLinks = [
    { name: "Անհատ", path: "/", base: "/" },
    { name: "Բիզնես", path: getFirstPath("/business"), base: "/business" },
    { name: "Ակնթարթային վճարումներ", path: getFirstPath("/instant-payments"), base: "/instant-payments" },
    { name: "Մեր մասին", path: getFirstPath("/about"), base: "/about" },
    { name: "Նորություններ", path: "/news", base: "/news" },
    { name: "Բլոգ", path: "/blog", base: "/blog" },
    { name: "Կարիերա", path: getFirstPath("/career"), base: "/career" },
  ];
      ...
      <div className="hidden xl:flex w-full h-10 border-b border-gray-100 bg-[#f8f9fb] justify-center">
        <div className="w-full max-w-[1450px] px-6 flex items-center justify-between">
          <div className="flex items-center h-full space-x-6">
            {topNavLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`text-[11px] font-black transition-all h-full flex items-center relative uppercase tracking-tighter ${currentTopPath === link.base ? "text-[#6610f2]" : "text-gray-400 hover:text-[#6610f2]"}`}
              >
                {link.name}
                {currentTopPath === link.base && (
          ...
          <div className="flex items-center space-x-6">
             ...
             <span className="text-[#6610f2] text-[11px] font-black uppercase tracking-widest cursor-pointer ml-4">EvocaONLINE</span>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="absolute inset-0 opacity-[0.12] pointer-events-none">
                <img 
                    ...
                    className="h-[110%] w-auto absolute -left-10 -bottom-10 rotate-[15deg] object-cover mix-blend-overlay" 
                    alt="" 
                />
            </div>
              ...
              <div className="w-full md:w-1/3 flex flex-col space-y-6 border-b md:border-b-0 md:border-r border-white/10 pb-10 md:pb-0">
                {topNavLinks.map((link, idx) => (
                  <motion.div key={link.name} initial={{ x: -30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: idx * 0.05 }}>
                    <button 
                      onMouseEnter={() => setActiveTab(link.base)}
                      onClick={() => navigate(link.path)}
                      className={`text-3xl md:text-5xl font-black italic uppercase tracking-tighter transition-all text-left ${activeTab === link.base ? 'text-white translate-x-4' : 'text-white/30 hover:text-white/60'}`}
                    >
                      {link.name}
                    </button>
                  </motion.div>
              ...
              <div className="w-full md:w-2/3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-8 md:pl-20 pt-10 md:pt-4 overflow-y-auto no-scrollbar">
                <AnimatePresence mode="wait">
                  <motion.div 
                    ...
                    className="contents"
                  >
                    {(secondaryMenus[activeTab] || []).map((sub) => (
