...
// const Hashivner: React.FC = () => {
//     const { t, i18n } = useTranslation(); // Hook-ի սահմանում
//     const [activeTab, setActiveTab] = useState(0);
//     const [openAccordion, setOpenAccordion] = useState<number | null>(null);
...
//     useEffect(() => {
//         // Տվյալները բեռնում ենք ըստ լեզվի (օրինակ՝ bizneshashivner_am)
// const lang = i18n.language.includes('hy') ? 'am' : i18n.language.split('-')[0];
// const hashivnerRef = ref(db, `bizneshashivner_${lang}`);
//         const unsubscribe = onValue(hashivnerRef, (snapshot) => {
//             const data = snapshot.val();
//             if (data) {
//                 if (data.sections) setSections(data.sections);
...
//         return () => unsubscribe();
//     }, [i18n.language]); // Լեզուն փոխելիս վերաբեռնում է
...
//             <div className="max-w-[1140px] mx-auto px-4 pt-10">
//                 <div className="flex items-center gap-2 text-[10px] text-gray-400 uppercase tracking-[0.2em] mb-8 font-bold">
//                     <span>{t('hashivner.breadcrumb_parent')}</span> <ChevronRight size={10} /> <span className="text-gray-800">{t('hashivner.title')}</span>
//                 </div>
//                 <h1 className="text-[36px] md:text-[50px] font-[900] italic uppercase leading-none mb-12 tracking-tighter">
//                     {t('hashivner.title')}
//                 </h1>
...
//                             {t('hashivner.online_banking')}
//                         </h2>
//                         <p className="text-white/80 mb-8 text-lg">
//                             {t('hashivner.online_desc')}
//                         </p>
//                         <motion.button 
//                             whileHover={{ scale: 1.1, boxShadow: "0px 0px 20px rgba(255,255,255,0.5)" }}
//                             whileTap={{ scale: 0.9 }}
...
//                     {testimonials.map((t, idx) => (
//                         <SwiperSlide key={idx}>
//                             <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8 }} className="text-center max-w-3xl mx-auto">
//                                 <div className="relative inline-block">
//                                     <span className="text-[80px] text-[#6610f2]/10 absolute -left-10 -top-10 font-serif">“</span>
//                                     <p className="text-[20px] md:text-[26px] font-medium italic leading-relaxed text-gray-700 relative z-10 px-6">{t.text}</p>
//                                     <span className="text-[80px] text-[#6610f2]/10 absolute -right-10 bottom-0 font-serif">”</span>
//                                 </div>
//                                 <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-10">
//                                     <h4 className="text-[#6610f2] font-black text-xl uppercase tracking-tighter">{t.name}</h4>
//                                     <p className="text-gray-400 text-xs uppercase tracking-[0.3em] mt-2">{t.role}</p>
//                                 </motion.div>
//                             </motion.div>
