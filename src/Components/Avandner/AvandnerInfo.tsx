  ...
  if (!deposit) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8f9fb]">
        <h2 className="text-xl font-bold mb-4">{t('deposits_page.not_found')}</h2>
        <button onClick={() => navigate(-1)} className="text-[#6610f2] font-bold uppercase underline">{t('deposits_page.back')}</button>
      </div>
    );
  }
    ...
    <div className="bg-white min-h-screen font-sans">
      <div className="max-w-[1200px] mx-auto px-4 pt-10">
        ...
        >
          ...
          <span className="text-[12px] font-black uppercase tracking-widest">{t('deposits_page.back')}</span>
        </button>
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 p-8 bg-[#f8f9fb] rounded-[30px] mb-10">
              <div className="flex flex-col">
                <span className="text-[#adb5bd] text-[11px] uppercase font-black tracking-widest mb-2">{t('deposits_page.min_amount')}</span>
                <div className="text-[#6610f2] text-[38px] font-[900] leading-none mb-1 tracking-tighter">{deposit.minAmount}</div>
                <span className="text-[#495057] text-[13px] font-bold uppercase">{deposit.amountLabel}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[#adb5bd] text-[11px] uppercase font-black tracking-widest mb-2">{t('deposits_page.annual_rate')}</span>
                <div className="text-[#6610f2] text-[38px] font-[900] leading-none mb-1 tracking-tighter">{deposit.rate}</div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-5 border border-gray-100 rounded-2xl hover:border-purple-200 transition-colors">
                <div className="bg-purple-100 p-2 rounded-full text-[#6610f2]"><Info size={20} /></div>
                <div>
                  <p className="text-[11px] font-black uppercase text-gray-400">{t('deposits_page.term')}</p>
                  <p className="text-[14px] font-bold text-gray-800">{t('deposits_page.term_value')}</p>
                </div>
              </div>
              <div className="flex items-center gap-4 p-5 border border-gray-100 rounded-2xl hover:border-purple-200 transition-colors">
                <div className="bg-purple-100 p-2 rounded-full text-[#6610f2]"><Info size={20} /></div>
                <div>
                  <p className="text-[11px] font-black uppercase text-gray-400">{t('deposits_page.payment_type')}</p>
                  <p className="text-[14px] font-bold text-gray-800">{t('deposits_page.payment_value')}</p>
                </div>
              </div>
            </div>
