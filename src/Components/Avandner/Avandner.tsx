...
const Avandner: React.FC = () => {
  ...
  // Փոփոխում ենք deposits-ը state-ի, որպեսզի տվյալները ստանանք բազայից
  const [deposits, setDeposits] = useState<DepositType[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Միանում ենք Firebase-ի 'deposits' ճյուղին
    ...
    const unsubscribe = onValue(depositsRef, (snapshot) => {
      ...
      if (data) {
        ...
        const depositsList = Object.keys(data).map(key => ({
          ...data[key],
          id: data[key].id || key // օգտագործում ենք key-ը որպես id, եթե չկա
        }));
    ...
    <div className="bg-white min-h-screen font-sans pb-20">
      ...
      <main className="max-w-[1200px] mx-auto px-4 mt-12 space-y-16">
        {deposits.length > 0 ? (
          ...
          <div className="text-center py-20 text-gray-400 font-bold">Ավանդներ չեն գտնվել:</div>
        )}
      </main>
    </div>
  );
