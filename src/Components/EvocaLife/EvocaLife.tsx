...
const EvocaLife: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(careerRef, (snapshot) => {
            ...
            if (data) {
                if (data.benefits) setBenefitIcons(Array.isArray(data.benefits) ? data.benefits : Object.values(data.benefits));
                if (data.faqs) setFaqs(Array.isArray(data.faqs) ? data.faqs : Object.values(data.faqs));
            }
            setLoading(false);
        });
    ...
    if (loading) return <div className="py-40 text-center font-[1000] italic text-[#6610f2] animate-pulse">ԲԵՌՆՎՈՒՄ Է...</div>;
