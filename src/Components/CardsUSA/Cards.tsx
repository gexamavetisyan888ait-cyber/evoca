...
const BestOffers: React.FC = () => {
  ...
  useEffect(() => {
    // Միանում ենք Firebase-ի 'best_offers' node-ին
    const offersRef = ref(db, 'cardUSA');
    
    const unsubscribe = onValue(offersRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // Եթե տվյալները Firebase-ում պահված են որպես օբյեկտների ցուցակ
          ...
          <div className="w-full lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-6 relative z-30">
            {!loading ? offers.map((item, idx) => (
              ...
              <div className="col-span-full text-center py-10">
                <div className="inline-block animate-bounce text-white font-black italic uppercase tracking-widest">
                  Սպասեք...
                </div>
              </div>
            )}
